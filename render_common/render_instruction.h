...
enum class RenderCmdType : uint8_t
{
    ...
    HEADER_FRAME = 42, // 页眉
    FOOTER_FRAME = 43, // 页脚
    FOOTNOTE_CONT_FRAME = 44, // 脚注容器
    FOOTNOTE_FRAME = 45, // 脚注
    FLY_FRAME = 46, // 浮动框
    RECT = 50, // 矩形 (背景/边框)
    LINE = 51, // 线段 (分隔线)
...
inline RenderCmdType RenderCmdTypeFromName(const char* name)
{
    ...
    switch (name[0])
    {
        ...
        case 'C':
            return RenderCmdType::COLUMN_FRAME;
        case 'H':
            return RenderCmdType::HEADER_FRAME;
        case 'F':
            if (name[1] == 'O')
            {
                // FOOTER_FRAME:    F O O T E R... → name[4] = 'E'
                // FOOTNOTE_CONT_FRAME: F O O T N O T E _ C... → name[8] = 'C'
                // FOOTNOTE_FRAME:  F O O T N O T E _ F... → name[8] = 'F'
                if (name[4] == 'E')
                    return RenderCmdType::FOOTER_FRAME;
                if (name[8] == 'C')
                    return RenderCmdType::FOOTNOTE_CONT_FRAME;
                return RenderCmdType::FOOTNOTE_FRAME;
            }
            if (name[1] == 'L')
                return RenderCmdType::FLY_FRAME;
            break;
        case 'R':
