...
void RenderInstructionOutputDevice::DrawText(const Point& rPt, const std::string& rText)
{
    ...
    BuildTextRunInstruction(
        m_rSink, m_nPageNum, rPt.x, rPt.y, rText.c_str(), static_cast<int>(rText.size()),
        m_aCurrentFont.familyName.c_str(), m_aCurrentFont.GetHeightInHalfPoints(),
        m_aTextColor.valid ? m_aTextColor.ToRGB() : 0, static_cast<uint8_t>(m_aCurrentFont.weight),
        static_cast<uint8_t>(m_aCurrentFont.italic));
}

void RenderInstructionOutputDevice::DrawRect(const SwRect& rRect)
{
    BuildRectInstruction(m_rSink, m_nPageNum, rRect.Left(), rRect.Top(), rRect.Width(),
                         rRect.Height());
}

void RenderInstructionOutputDevice::DrawLine(const Point& rStart, const Point& rEnd)
{
    BuildLineInstruction(m_rSink, m_nPageNum, rStart.x, rStart.y, rEnd.x, rEnd.y);
}

void RenderInstructionOutputDevice::DrawBitmap(const Point& rPt, const Size& rSize)
{
    BuildBitmapInstruction(m_rSink, m_nPageNum, rPt.x, rPt.y, rSize.width, rSize.height);
}

void RenderInstructionOutputDevice::DrawEllipse(const SwRect& rRect)
{
    BuildEllipseInstruction(m_rSink, m_nPageNum, rRect.Left(), rRect.Top(), rRect.Width(),
                            rRect.Height());
}
...
void RenderInstructionOutputDevice::SetFont(const OutputFont& rFont)
{
    ...
    BuildSetFontInstruction(m_rSink, m_nPageNum, m_aCurrentFont.familyName.c_str(),
...
{
    m_aTextColor = rColor;
    BuildSetTextColorInstruction(m_rSink, m_nPageNum, rColor.valid ? rColor.ToRGB() : 0);
}
...
void RenderInstructionOutputDevice::SetFillColor(const OutputColor& rColor)
{
    ...
    BuildSetFillColorInstruction(m_rSink, m_nPageNum, rColor.valid ? rColor.ToRGB() : 0);
}
...
void RenderInstructionOutputDevice::SetLineColor(const OutputColor& rColor)
{
    ...
    BuildSetLineColorInstruction(m_rSink, m_nPageNum, rColor.valid ? rColor.ToRGB() : 0);
}

void RenderInstructionOutputDevice::SetClipRegion(const SwRect& rRect)
{
    (void)rRect; // 裁剪区域暂不记录几何
    BuildSetClipRegionInstruction(m_rSink, m_nPageNum);
}
...
void RenderInstructionOutputDevice::Push() { BuildPushInstruction(m_rSink, m_nPageNum); }

void RenderInstructionOutputDevice::Pop() { BuildPopInstruction(m_rSink, m_nPageNum); }
