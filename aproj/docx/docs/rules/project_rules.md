...
> - **架构照搬**：模块划分、类层次、调用链路必须与 LO 一致，不得自行重构架构
> - **数据结构照搬**：SwDoc、SwNodes、SwNode、SwFrame 等核心数据结构的定义、字段、关系必须与 LO 一致，不得自行增删字段或改变继承关系
> - **算法逻辑照搬**：排版计算、分页决策、断行逻辑、浮动对象定位等算法必须从 LO 源码迁移，不得自行设计替代算法
> - **命名照搬**：类名、方法名、变量名保持与 LO 一致，便于对照溯源
>
> 简而言之：**LO 怎么写，aproj/docx 就怎么写**。任何"我觉得这样更好"的改动都是违反项目原则的。
...
2. **最小实现但功能完整**: 迁移 LibreOffice 关于 DOCX 文档核心最小实现，但功能要完整。"最小"指裁剪非 DOCX 相关功能，不是指简化已有逻辑
...
8. **架构/数据结构/算法不允许自行设计**: 这是第 5-7 条的总纲——aproj/docx 的类层次、数据结构、算法逻辑必须与 LO 源码一致。遇到任何需要"设计"的场景，答案永远是"去看 LO 怎么做的，照搬过来"

### 违反处理

- 硬编码或自行实现的逻辑：**必须删除重写**，不得以"能用就行"为由保留
- 未参考 LO 源码的猜测性实现：**要求回溯验证**，对照 LO 源码确认逻辑一致性后方可合入
- 架构/数据结构与 LO 不一致：**必须回退到 LO 方案**，不得以"更简洁"或"更合理"为由保留自行设计
...
- **临时分析文件**: 大模型在分析过程中创建的临时脚本、探查代码、中间分析文件等，一律放在 `aproj/docx/tools/debug` 下面，不要散落在项目其他目录中。分析完成后无需保留的文件也在此目录清理
...
| **test_diff_workflow** | `skills/test_diff_workflow/SKILL.md` | 测试差异对比与迭代修复工作流：调用 `gen_*.py` 生成产物、调用 `diff_*.bat` 对比差异、三级差异对比（Nodes→Frame→VCL）、差异定位与迭代修复 |
